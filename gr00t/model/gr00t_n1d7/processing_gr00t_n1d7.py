...
# Repository-specific changes are described in the repository README.
...
class Gr00tN1d7Processor(BaseProcessor):
    ...
    def __init__(
        ...
        # Mixed-time ablation: current pressure, five arm-joint frames, current grippers.
        pack_current_pressure_then_joint_history: bool = False,
        # Corrected timing: current pressure, five full 16D proprioception frames.
        pack_current_pressure_then_proprio_history: bool = False,
    ):
        ...
        self.pack_current_pressure_then_joint_history = (
            pack_current_pressure_then_joint_history
        )
        self.pack_current_pressure_then_proprio_history = (
            pack_current_pressure_then_proprio_history
        )
        if (
            self.pack_current_pressure_then_joint_history
            and self.pack_current_pressure_then_proprio_history
        ):
            raise ValueError("Only one mixed-time state packing mode may be enabled")
        ...
        if isinstance(image_target_size, dict):
            # Per-view image preprocessing: each camera may use its own target size.
            self.train_image_transform = {}
            self.eval_image_transform = {}
            for view, view_target_size in image_target_size.items():
                view_crop_size = (
                    image_crop_size.get(view)
                    if isinstance(image_crop_size, dict)
                    else image_crop_size
                )
                if use_albumentations:
                    train_tf, eval_tf = build_image_transformations_albumentations(
                        view_target_size,
                        view_crop_size,
                    ...
                    train_tf, eval_tf = build_image_transformations(
                        view_target_size,
                        view_crop_size,
                        random_rotation_angle,
                        color_jitter_params,
                ...
                self.train_image_transform[view] = train_tf
                self.eval_image_transform[view] = eval_tf
    ...
    def _pad_or_pack_states(
        self, normalized_states: torch.Tensor, state_keys: list[str]
    ) -> torch.Tensor:
        """Pad ordinary states or build one of the mixed-time vectors.

        The mixed-time representation has exactly 74 active values in this order:
        [L2_t, R2_t, arm_joints_{t-4}(14), ..., arm_joints_t(14), grippers_t(2)].
        It is placed at the
        start of the model's existing 5 * max_state_dim flattened state buffer so
        model architecture and checkpoint shapes remain identical to the other
        five-frame experiment.
        """
        if not (
            self.pack_current_pressure_then_joint_history
            or self.pack_current_pressure_then_proprio_history
        ):
            ...
            return torch.cat([normalized_states, normalized_states.new_zeros(padding_shape)], dim=-1)
        ...
        if list(state_keys) != expected_keys:
            raise ValueError(
                "Current-pressure packing requires state keys in this exact order: "
                f"{expected_keys}; got {list(state_keys)}"
            )
        if normalized_states.ndim not in (2, 3):
            raise ValueError(
                "Current-pressure packing expects [T,D] or [B,T,D], got "
                f"{tuple(normalized_states.shape)}"
            )
        if normalized_states.shape[-2] != 5 or normalized_states.shape[-1] != 18:
            raise ValueError(
                "Current-pressure packing requires five 18D source frames, got "
                f"{tuple(normalized_states.shape)}"
            )

        if self.pack_current_pressure_then_proprio_history:
            if normalized_states.ndim == 2:
                current_pressure = normalized_states[-1, :2]
                proprio_history = normalized_states[:, 2:18].reshape(-1)
                active = torch.cat([current_pressure, proprio_history], dim=-1)
                packed = normalized_states.new_zeros(5 * self.max_state_dim)
                packed[: active.numel()] = active
                return packed.reshape(5, self.max_state_dim)

            batch_size = normalized_states.shape[0]
            current_pressure = normalized_states[:, -1, :2]
            proprio_history = normalized_states[:, :, 2:18].reshape(batch_size, -1)
            active = torch.cat([current_pressure, proprio_history], dim=-1)
            packed = normalized_states.new_zeros(batch_size, 5 * self.max_state_dim)
            packed[:, : active.shape[-1]] = active
            return packed.reshape(batch_size, 5, self.max_state_dim)

        if normalized_states.ndim == 2:
            current_pressure = normalized_states[-1, :2]
            joint_history = normalized_states[:, 2:16].reshape(-1)
            current_grippers = normalized_states[-1, 16:18]
            active = torch.cat([current_pressure, joint_history, current_grippers], dim=-1)
            packed = normalized_states.new_zeros(5 * self.max_state_dim)
            packed[: active.numel()] = active
            return packed.reshape(5, self.max_state_dim)
        ...
        batch_size = normalized_states.shape[0]
        current_pressure = normalized_states[:, -1, :2]
        joint_history = normalized_states[:, :, 2:16].reshape(batch_size, -1)
        current_grippers = normalized_states[:, -1, 16:18]
        active = torch.cat([current_pressure, joint_history, current_grippers], dim=-1)
        packed = normalized_states.new_zeros(batch_size, 5 * self.max_state_dim)
        packed[:, : active.shape[-1]] = active
    ...
    def process_observation(self, observation: dict[str, Any], embodiment_tag: EmbodimentTag):
        ...
        normalized_states = self._pad_or_pack_states(normalized_states, state_keys)
    ...
    def _get_vlm_inputs(
        ...
        if isinstance(image_transform, dict):
            transforms_by_view = image_transform
        else:
            transforms_by_view = {view: image_transform for view in image_keys}

        if self.use_albumentations:
            # Each view gets its own transform/replay so output sizes can differ.
            for view in image_keys:
                ...
                view_transform = transforms_by_view[view]
                transformed_images, _ = apply_with_replay(
                    view_transform, images[view], view_masks, replay=None
                )
        ...
        # Keep views as a list instead of stacking them, because target sizes may differ.
        frames = []
        for view in image_keys:
            frames.extend(list(temporal_stacked_images[view]))
        vlm_inputs = self._apply_vlm_processing(frames, language)
        return vlm_inputs
